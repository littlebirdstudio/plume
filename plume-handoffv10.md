...
- **Ingredient cheat sheet — remaining steps.** Steps 1-4 done (types in core,
  multi-function, purpose, search). Next is step 5, the comparison chart grouped
  by function with per-group columns; then step 6, the product-type view
  derived from her own formulations. Spec below under "Spec: ingredient cheat
  sheet".
